      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            {
              label: "EMA 21",
              value: snapshot?.ema21?.toFixed(2) ?? "--",
              color: "text-[#40c4ff]",
            },
            ...
            <div
              key={label}
              className="flex flex-col items-center bg-muted/40 border border-border rounded-lg py-2.5 px-2"
            >
              <span className="text-[9px] uppercase tracking-widest text-muted-foreground font-semibold mb-1">
                {label}
              </span>
              <span className={cn("text-base font-bold font-mono", color)}>
                {loading ? (
                  <span className="inline-block h-4 w-12 rounded animate-pulse bg-muted" />
                ) : (
                  value
                )}
              </span>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span
            className={cn(
              "text-sm font-extrabold font-mono px-3 py-1 rounded-md",
              SIGNAL_STYLES[snapshot?.signal ?? "WAIT"],
            )}
          >
          ...
          <span
            className={cn(
              "text-[11px] font-bold rounded-full px-2.5 py-0.5 uppercase tracking-wider",
              CONDITION_STYLES[snapshot?.marketCondition ?? "Neutral"],
            )}
          >
        ...
        <p className="text-xs text-muted-foreground italic leading-relaxed">
          {loading ? (
            <span className="inline-block h-4 w-full rounded animate-pulse bg-muted" />
          ) : (
            (snapshot?.reason ?? "Loading analysis…")
          )}
        </p>

        {/* Confidence bar */}
        <div>
          <span className="text-[9px] uppercase tracking-widest text-muted-foreground font-bold mb-1.5 block">
            Confidence
          </span>
          <div className="flex items-center gap-2">
            <div className="flex-1 h-1.5 rounded-full bg-muted overflow-hidden">
              <div
                className={cn(
                  ...
                  BAR_COLORS[snapshot?.signal ?? "WAIT"],
                )}
