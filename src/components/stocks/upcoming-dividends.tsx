...
import { daysUntilCalendarDate, formatCalendarDate, formatRs } from "@/lib/format";
...
function CountdownBadge({ recordDate }: { recordDate: string }) {
  const days = daysUntilCalendarDate(recordDate);
  ...
  return (
    ...
    >
      ...
      {formatCalendarDate(recordDate)} · {label}
    </span>
  );
}
